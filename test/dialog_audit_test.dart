// 彈窗長相對照：把 App 裡實際會出現的幾種彈窗各拍一張，
// 放在一起看就知道哪裡不一致。這不是回歸測試，是給人看的。
//
//   flutter test --update-goldens test/dialog_audit_test.dart
//
// 產生／更新圖片：
...
void main() {
  ...
  testWidgets('彈窗 A：離開確認（D 款）', (tester) async {
    // 直接叫 App 用的那支（theme.dart 的 showLeaveChoice），
    // 跟編輯畫面按返回時跑的是同一段程式碼
    ...
    await expectLater(
      ...
      matchesGoldenFile('goldens/audit_leave.png'),
    );
  });

  testWidgets('彈窗 B：畫質選單（AlertDialog）', (tester) async {
    await tester.pumpWidget(
      MaterialApp(
        theme: buildStudioTheme(),
        debugShowCheckedModeBanner: false,
    ...
    await expectLater(
      find.byType(MaterialApp),
      matchesGoldenFile('goldens/audit_quality.png'),
    );
  });

  testWidgets('彈窗 D：每秒張數', (tester) async {
    await tester.pumpWidget(
      MaterialApp(
        theme: buildStudioTheme(),
        debugShowCheckedModeBanner: false,
    ...
    await tester.tap(find.text('匯出').last);
    await _settle(tester);
    await tester.tap(find.text('每秒張數'));
    await _settle(tester);
    await expectLater(
      find.byType(MaterialApp),
      matchesGoldenFile('goldens/audit_fps.png'),
    );
  });
  ...
  testWidgets('彈窗 C：畫面比例（AlertDialog）', (tester) async {
    ...
    await tester.tap(find.text('匯出').last);
    await _settle(tester);
    await tester.tap(find.text('畫面比例'));
    await _settle(tester);
    await expectLater(
      find.byType(MaterialApp),
      matchesGoldenFile('goldens/audit_ratio.png'),
    );
  });
}
