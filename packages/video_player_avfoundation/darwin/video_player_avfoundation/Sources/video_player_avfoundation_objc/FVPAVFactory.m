...
- (AVPlayer *)playerWithPlayerItem:(NSObject<FVPAVPlayerItem> *)playerItem {
  ...
  // MarkCut patch: local files never stall, but the default YES makes
  // AVPlayer wait for "enough buffer" before rolling — measured 267ms+
  // between play() and the first frame on 4K HDR clips. Kill the wait.
