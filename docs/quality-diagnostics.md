## 2026-09-24 效能一輪＋草稿夾容量（perf-storage-1）

BUILD 219（blob-store-1）實機：5 支 4K HDR 編輯中 ~1.0GB、剩 2.3GB，使用者確認
閃退與舊問題都沒了。報告裡剩下的：起播 P95 377ms（只有第一次，其他 ~190ms）、
播放器建置平均 124ms／最久 255ms（**整段在跟 Flutter 共用的主執行緒上**）。

- **組合成丟背景**：原生 `build` 在 `markcut.comp.build` 佇列組資料（開素材、
  鋪軌、驗指令、建 AVPlayerItem），只有 `commitBuild`（PlayerHosts、換 item、
  材質）回主執行緒。組建期間其他合成通道指令排隊，換上後照原順序補做——跟以前
  「卡在組建後面」的順序一樣，只是畫面不凍。回報多了 `queuedMs／prepareMs／
  commitMs`；新指標「播放器換上」只量主執行緒那段（目標 16ms）。原本
  「播放器建置」照量，但它現在不代表卡畫面。
- 組建裡拿掉每次兩遍的 VM 區域掃描（飛行紀錄器本來就每兩秒記）、紀錄檔寫入
  搬出主執行緒；`MCPreviewTail.lastSample`（每層開 AVAssetReader 讀最後一秒）
  與 `isHDRSource` 依檔案身分（路徑＋大小＋修改時間）快取。
- **起播不等換代理**：背景代理剛落地、只差素材路徑時按播放，直接播現在這顆
  （內容一格都沒錯），暫停後補重組。377ms 那筆一大半就是在等這次重組。
  Diag 計數「起播不等換代理」。
- **自動存檔瘦身**：封面不再塞進草稿內容（以前每次存檔多寫一張 720p PNG）；
  封面改 JPEG（原生 ImageIO，1.2MB→一百多 KB），同一張不重寫；浮水印直接畫進
  封面（省一次 PNG 編解碼）；抽過的影格重用、被整片蓋住的下層不抽（多選匯入
  五層疊在 0 秒只抽最上面一層）；內容編碼＋寫檔整段在背景 isolate。
  BlobStore 改非同步 I/O，同一個鍵的讀寫刪排成一列。
- 照片／批次／拼圖／GIF 四個單頁草稿也搬出 SharedPreferences。
- **草稿夾容量**：最上面「佔用空間」卡（影片草稿、轉檔暫存、其中沒有草稿在用的
  ＋「清掉」）；選取模式每份顯示「刪掉能省多少」（共用的不算），刪除鈕寫總共
  省多少。每份草稿存檔時另寫一個很小的檔案清單（`project_refs_*`），舊草稿由
  背景 isolate 讀內容補算一次。**刪草稿連帶清只有它在用的轉檔暫存**（以前只刪
  兩個小檔，幾百 MB 的代理留著）；剩下的草稿有一份沒有清單就整個不清。
  順手修 `WorkFiles.releaseFiles` 沒問 `referenced(路徑)`：別份草稿直接指著的
  救回素材（imports/）會被刪。舊草稿的 PNG 封面在草稿夾開著時一張張換成 JPEG。

驗收：下一份報告「播放器換上」應在個位數毫秒；「起播確認」不再有 300ms 以上
的單筆（除非真的在重組）；草稿夾看得到容量、清掉後數字下降。
已知：`export_tab`／`export_quality_sheet`／`gif_add_menu`／`reorder_sheet`
四張 golden 在 b055a89 本身就差 0.05～0.07%（這一輪沒動它們）。
...
- 看的地方：首頁橫幅（標題照死因寫：程式例外／系統因記憶體關閉／其他異常／
  處理中斷，「查看報告」→「複製報告」）；編輯器右上角品質診斷器最上面的
  「上次結束原因」卡片，複製的驗收報告與 JSON（`exitReason`）也帶著；標題列
  長按的播放診斷「出報告」。MetricKit 開 App 後才送到，所以開 App 8 秒、
  60 秒各補查一次，品質診斷器每次更新也查；同一份異常只跳一次橫幅
  （`exit_seen.txt`），已有中斷橫幅就補進那份報告。
...
上一輪 AAC 設定修正及模擬器通過，均不能證明已解決此實機問題。
...
- Dart marker 使用世代號與原子換檔，避免延遲寫入形成假中斷紀錄；
  完整內容在非同步暫存檔寫完，最後只同步替換小型目錄項目。
...
原生整合測試另涵蓋系統未在 250ms 內回覆 composition copy 的情況：允許
既有單次 seek 備援，但必須觀察到同一播放器完成最新樣式世代的 CI 影格，
且沒有殘留工作或多次 seek。`previewRenderedEpoch/Seconds` 是每個播放器
最近完成的合成影格（包括備援），只記兩個數值，不代表實際顯示時間；播放
期間沒有新增主執行緒回呼。此測試不再以共享 CI 主機必須達成 250ms 作為
正確性斷言；沒有備援時仍要求完全不 seek、播放時間不變。
