...
class TimelineModel {
  ...
  /// 手動按「銜接」走這條，自動銜接（_autoTidyIfOn）也走這條——
  /// 使用者回報「銜接開著時拖最前面那支的左把手往右，前面空出來的
  /// 沒有補到最開始」，兩邊本來不一致。代價講明：銜接開著的時候，
  /// 片頭刻意留白（等音樂進來）的排法會在下一次編輯被吸回 0，
  /// 要留白就把「銜接」關掉。不給 fromZero 就只補片段之間的洞
