...
class WorkFiles {
  ...
  /// HDR 代理的短邊上限。SDR 工作檔那條維持 1080（它餵匯出，不能動），
  /// 代理只餵預覽跟縮圖：合成畫布實測 720x1280（202 診斷）與 900x1600
  /// （201 診斷）都在 900 以內，1080 是白編的像素——短邊 1080→900
  /// 每格少編約三成，進場遮罩跟著變短（202：代理 3 支佔掉進場 41 秒
  /// 的全部，工作檔 0 支）。
  ///
  /// 為什麼不直接降到 720：畫布是照「最底層最早那段轉正後的尺寸」算的，
  /// 實際上跟預覽窗格在該裝置的像素大小綁在一起，不是固定值。900 是
  /// 兩份實測都不會變糊的保守值。要再往下就得在準備階段先拿到畫布尺寸、
  /// 並把它寫進索引鍵（畫布變大要重轉），那是另一件事。
  ///
  /// 動它安全的三個理由（改之前查過）：匯出讀原檔、不碰代理（編輯器的
  /// 「匯出來源：原檔」）；原生端 alreadyGoodEnough 對 HDR 素材一律回
  /// 「必須轉」，不會因為門檻放寬就跳過轉檔而失去密關鍵幀；索引鍵刻意
  /// 不動，既有的 1080 代理照樣命中重用——換鍵等於逼所有人把要省的
  /// 時間再付一次
  static const kHdrProxyShortSide = 900;

  /// HDR 代理（HLG 直通、密關鍵幀）。HDR 預覽模式播它——
  /// SDR 工作檔在那個模式下永遠比成品淡，原檔又是 4K 解起來重。
  /// 索引 key 用「原檔路徑#hdr6」，跟 SDR 工作檔各記各的。
  /// #hdr（v106）那一批像素是內建合成器轉壞的（標 HLG 裝 SDR），
  /// 換 key 整批作廢，舊檔變孤兒由 sweep 清掉
  static Future<String?> lookupHdr(String src) async {
    if (kIsWeb) return null;
    final idx = await _load();
    final e = idx['$src#hdr6'];
    if (e is! Map) return null;
    final work = e['work'] as String?;
    if (work == null || !File(work).existsSync()) return null;
    final stamp = _stamp(src);
